...
Every `assert` below is flagged by Bandit's B101 ("assert is stripped under
python -O") and suppressed with `# nosec B101`: these are pytest assertions,
not runtime checks, this file never ships, and it only ever runs under
`pytest`, which never invokes -O.
"""
...
def test_crops_to_the_exact_bounding_box_of_the_non_transparent_pixels():
    ...
    bbox = bbox_of(result)
    assert (bbox[2] - bbox[0], bbox[3] - bbox[1]) == (600, 300)  # nosec B101
...
def test_gives_a_wide_garment_exactly_margin_on_its_sides():
    ...
    left = bbox[0] / w
    right = (w - bbox[2]) / w
    assert left == pytest.approx(MARGIN, abs=0.005)  # nosec B101
    assert right == pytest.approx(MARGIN, abs=0.005)  # nosec B101
...
def test_gives_a_tall_garment_exactly_margin_top_and_bottom():
    ...
    top = bbox[1] / h
    bottom = (h - bbox[3]) / h
    assert top == pytest.approx(MARGIN, abs=0.005)  # nosec B101
    assert bottom == pytest.approx(MARGIN, abs=0.005)  # nosec B101
...
def test_output_is_target_aspect():
    ...
    result = framed(cutout)
    w, h = result.size
    assert w / h == pytest.approx(TARGET_ASPECT, abs=0.005)  # nosec B101
...
def test_centres_the_garment_on_the_new_canvas():
    ...
    top = bbox[1]
    bottom = h - bbox[3]
    assert left == pytest.approx(right, abs=1)  # nosec B101
    assert top == pytest.approx(bottom, abs=1)  # nosec B101
...
def test_passes_through_unchanged_when_nothing_is_opaque():
    ...
    assert frame_cutout(original, margin=MARGIN, target_aspect=TARGET_ASPECT) == original  # nosec B101
